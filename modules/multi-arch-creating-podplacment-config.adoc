...
`spec.logVerbosityLevel`:: Optional: Specifies the log verbosity level. You can set the field value to `Normal`, `Debug`, `Trace`, or `TraceAll`. The value is set to `Normal` by default.
...
`spec.fallbackArchitecture`:: Optional: Specifies an architecture where pods will be scheduled if the image inspector cannot determine the architecture of the image. Valid values are `arm64`, `amd64`, `ppc64le`, or `s390x`.
