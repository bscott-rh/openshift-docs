...
----
. Modify the `iam.allowedPolicyMemberDomains` constraint to allow the service account that the installation program uses to authenticate with {gcp-short} and create storage as in the following example:
+
[source,yaml]
----
