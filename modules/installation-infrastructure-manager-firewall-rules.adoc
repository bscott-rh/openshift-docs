// Module included in the following assemblies:
//
// * installing/installing_gcp/installing-gcp-user-infra-shared-vpc.adoc

:_mod-docs-content-type: REFERENCE
[id="installation-infrastructure-manager-firewall-rules_{context}"]
= Infrastructure Manager template for firewall rules

[role="_abstract"]
You can use the following Infrastructure Manager template to deploy the firewall rules that you need for your {product-title} cluster:

.`03_security.tf` Infrastructure Manager template
[%collapsible]
====
[source,python]
----
include::https://raw.githubusercontent.com/openshift/installer/release-4.22/upi/gcp/03_security/03_security.tf[]
----
====
